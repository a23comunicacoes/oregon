...
export const useCookieStore = defineStore('cookie', {
  state: () => ({
    maxAge: 25 * 24 * 60 * 60,
    userData: useCookie('userData').value, // Expira em
    userAbilityRules: useCookie('userAbilityRules').value,
    accessToken: useCookie('accessToken').value,
  }),
  actions: {
    updateCookies({ userData, rules, accessToken, maxAge }) {
      //Expirar cookies em 2 dias
      let cookieOptions = { maxAge: maxAge || this.maxAge };

      // Atualiza os valores dos cookies com as opções de maxAge
      useCookie('userData', cookieOptions).value = userData;
      useCookie('userAbilityRules', cookieOptions).value = rules;
