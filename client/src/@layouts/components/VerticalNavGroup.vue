<script setup>
  ...
  import { canViewNavMenuGroup, can } from "@layouts/plugins/casl";
...
<template>
  <li
    v-if="canViewNavMenuGroup(item) && checkCanGeral(item.subjectGeral)"
    class="nav-group"
    :class="[
      {
  ...
  >
    <div class="nav-group-label" @click="isGroupOpen = !isGroupOpen">
      ...
      <Component
        ...
        v-bind="
          !isMounted ? { class: 'd-flex align-center flex-grow-1' } : undefined
        "
      >
        <!-- 👉 Title -->
        <Component
