/**
 * Utilitário para validar configurações de IA do GPT (OpenAI)
...
export async function verificarConfiguracoesIA() {
  try {
    const response = await $api('/config/g/gpt_key');
    const gptKey = response && response.length > 0 ? response[0].value : null;
    
    if (!gptKey || gptKey === '' || gptKey === 'null') {
      return {
        configured: false,
        missingFields: ['gpt_key'],
        warnings: ['API Key da OpenAI não configurada']
      };
    }

    // Verificar outras configurações obrigatórias
    const configs = [
      { key: 'gpt_comportamento', name: 'Comportamento da IA' },
      { key: 'gpt_empresa', name: 'Informações da Empresa' },
      { key: 'gpt_agendamentos', name: 'Configurações de Agendamentos' }
    ];
...
export function mostrarErroConfiguracaoIA(validation, setAlert) {
  if (!validation.configured) {
    ...
    setAlert(
      `Para usar blocos de IA, você precisa configurar o GPT em "Configurações CRM > Atendente Virtual".\n\n${mensagem}`,
      'warning',
...
export function getMensagemAjuda(field) {
  const mensagens = {
    gpt_key: 'Configure a API Key da OpenAI em "Configurações CRM > Atendente Virtual > API"',
    gpt_comportamento: 'Configure o comportamento da IA em "Configurações CRM > Atendente Virtual > Comportamento"',
    gpt_empresa: 'Configure as informações da empresa em "Configurações CRM > Atendente Virtual > Empresa"',
    gpt_agendamentos: 'Configure as regras de agendamento em "Configurações CRM > Atendente Virtual > Agendamentos"'
  };
  
  return mensagens[field] || `Configure ${field} em "Configurações CRM > Atendente Virtual"`;
}
