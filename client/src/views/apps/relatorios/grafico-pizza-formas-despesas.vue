...
const chartOptions = computed(() => {
  ...
  return {
    ...
    labels: props.formasPagamentoDespesas?.map(f => f.forma) || [],
    colors: ['#EA5455', '#FF9F43', '#FFA1A1', '#FF6B6B', '#E74C3C', '#C0392B'],
    legend: {
      position: 'bottom',
      labels: {
        colors: textColor,
...
const series = computed(() => props.formasPagamentoDespesas?.map(f => f.valor) || [])
</script>
  ...
  <VCard>
    <VCardText>
      <div class="d-flex justify-space-between align-center mb-4">
        <div>
          <h5 class="text-h5 mb-1">Formas de Pagamento - Despesas</h5>
      ...
      <VueApexCharts 
        v-if="props.formasPagamentoDespesas && props.formasPagamentoDespesas.length > 0"
        :key="JSON.stringify(props.formasPagamentoDespesas)"
