...
//console.log('EmpresaData:', props.userData)
...
const onSubmit = () => {
  ...
  emit('userData', {
    ...
    email: email.value,
    phone: phone.value,
    isGeneratePassword: isGeneratePassword.value,
    color: color.value,
    expIni: expIni.value,
    expFim: expFim.value,
...
watch(avatar, async newAvatar => {
  if (newAvatar && newAvatar.length > 0 && newAvatar[0] instanceof File) {
    try {
      ...
      console.error('Erro ao converter o arquivo para URL de dados')
    }
  } else {
          ...
          >
            <VRow>
              ...
              <VDialog v-model="colorShow" persistent width="auto">
                <VCard class="pa-3">
                  <VColorPicker v-model="color" class="ma-2" :swatches="swatches" show-swatches
                    hide-inputs :modes="['hexa']" />
                  <VBtn :color="color ? color : '#3182CE'" @click="colorShow = !colorShow">
                      ...
                      >
                        ...
                        >
                          <p class="text-caption mt-2 mb-0 text-center w-100">
                            Será enviado um email para o email preenchido acima com um link para o usuário definir a senha
                            de sua
                            conta. <strong>O email tem validade de 24 horas!</strong>
                          </p>
                        </VCol>
                      </VRow>
                    </VRow>
              ...
              <VCol cols="12">
                <VBtn
                  ...
                  :append-icon="isGeneratePassword ? 'tabler-send' : ''"
                >
                  {{ isGeneratePassword ? 'Adicionar e Enviar' : 'Adicionar' }}
                </VBtn>
                <VBtn
                  type="reset"
                  variant="outlined"
